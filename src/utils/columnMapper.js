...
export const autoDetectColumns = (headers, mode) => {
  ...
  // Fields to detect based on mode (including currency if present in file)
...
export const getFieldsForMode = (mode) => {
  // Currency is optional - if mapped from file, it's used; otherwise falls back to Default Currency setting
  if (mode === 'standard') {
    return [
      ...
      { name: 'currency', label: 'Currency', required: false, note: 'Falls back to Default Currency if not mapped' }
    ];
  }
